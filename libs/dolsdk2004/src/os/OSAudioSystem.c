...
void __OSInitAudioSystem(void) {
  ...
  memcpy((void*)((u32)OSGetArenaHi() - 0x80), __DSPWorkBuffer, 128);
  memcpy(__DSPWorkBuffer, (void*)DSPInitCode, 128);
  DCFlushRange(__DSPWorkBuffer, 128);

  __DSPRegs[9] = 0x43;
  ASSERTMSGLINE(0x71, !(__DSPRegs[5] & 0x200), "__OSInitAudioSystem(): ARAM DMA already in progress");
  ASSERTMSGLINE(0x75, !(__DSPRegs[5] & 0x400), "__OSInitAudioSystem(): DSP DMA already in progress");
  ASSERTMSGLINE(0x79, (__DSPRegs[5] & 0x004), "__OSInitAudioSystem(): DSP already working");
  __DSPRegs[5] = 0x8AC;
  __DSPRegs[5] |= 1;
  while (__DSPRegs[5] & 1);
  __DSPRegs[0] = 0;
  ...
  *(u32*)&__DSPRegs[20] = 0x20;
  ...
  if(((u32)((reg16 << 16) | __DSPRegs[3]) + 0x7FAC0000U) != 0x4348) {
      ASSERTMSGLINE(0xC1, 0, "__OSInitAudioSystem(): DSP returns invalid message");
  }
  ...
  memcpy(__DSPWorkBuffer, (void*)((u32)OSGetArenaHi() - 0x80), 128);
}

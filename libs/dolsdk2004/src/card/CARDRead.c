...
s32 __CARDSeek(CARDFileInfo *fileInfo, s32 length, s32 offset, CARDControl **pcard) {
    ...
    ASSERTLINE(0x62, 0 <= fileInfo->chan && fileInfo->chan < 2);
    ASSERTLINE(0x63, 0 <= fileInfo->fileNo && fileInfo->fileNo < CARD_MAX_FILE);

    result = __CARDGetControlBlock(fileInfo->chan, &card);
    if (result < 0)
        return result;

    ASSERTLINE(0x6A, CARDIsValidBlockNo(card, fileInfo->iBlock));
    ASSERTLINE(0x6B, fileInfo->offset < card->cBlock * card->sectorSize);
    ...
    ASSERTLINE(0x75, ent->gameName[0] != 0xff);
...
static void ReadCallback(s32 chan, s32 result) {
    ...
    ASSERTLINE(0xC7, OFFSET(fileInfo->length, CARD_SEG_SIZE) == 0);
    ASSERTLINE(0xC8, OFFSET(fileInfo->offset, card->sectorSize) == 0);
...
s32 CARDReadAsync(CARDFileInfo *fileInfo, void *buf, s32 length, s32 offset, CARDCallback callback) {
    ...
    ASSERTLINE(0xFA, buf && OFFSET(buf, 32) == 0);
    ASSERTLINE(0xFB, OFFSET(offset, CARD_SEG_SIZE) == 0);
    ASSERTLINE(0xFC, 0 < length && OFFSET(length, CARD_SEG_SIZE) == 0);
...
s32 CARDCancel(CARDFileInfo *fileInfo) {
    ...
    ASSERTLINE(0x152, 0 <= fileInfo->chan && fileInfo->chan < 2);
    ASSERTLINE(0x153, 0 <= fileInfo->fileNo && fileInfo->fileNo < CARD_MAX_FILE);

...
s32 CARDFastOpen(s32 chan, s32 fileNo, CARDFileInfo *fileInfo) {
    ...
    ASSERTLINE(0x116, 0 <= fileNo && fileNo < CARD_MAX_FILE);
    ASSERTLINE(0x117, 0 <= chan && chan < 2);
...
s32 CARDOpen(s32 chan, char *fileName, CARDFileInfo *fileInfo) {
    ...
    ASSERTLINE(0x150, 0 <= chan && chan < 2);
...
s32 CARDClose(CARDFileInfo *fileInfo) {
    ...
    ASSERTLINE(0x17C, 0 <= fileInfo->chan && fileInfo->chan < 2);
    ASSERTLINE(0x17D, 0 <= fileInfo->fileNo && fileInfo->fileNo < CARD_MAX_FILE);

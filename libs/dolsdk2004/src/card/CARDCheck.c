...
void __CARDCheckSum(void *ptr, int length, u16 *checksum, u16 *checksumInv) {
    ...
    ASSERTLINE(0x52, length % sizeof(u16) == 0);
...
s32 __CARDVerify(CARDControl *card) {
    ...
    {
    case 0: 
        ASSERTLINE(0x12D, card->currentDir);
        ASSERTLINE(0x12E, card->currentFat);
        return CARD_RESULT_READY;
...
s32 CARDCheckExAsync(s32 chan, s32* xferBytes, CARDCallback callback) {
    ...
    ASSERTLINE(0x15A, 0 <= chan && chan < 2);
    ...
    ASSERTLINE(0x179, errors == 0 || errors == 1);

    switch (errors) {
    case 0:
        ASSERTLINE(0x17D, card->currentDir);
        ASSERTLINE(0x17E, card->currentFat);
        break;
    case 1:
        if (!card->currentDir) {
            ASSERTLINE(0x183, card->currentFat);
            card->currentDir = dir[currentDir];
            memcpy(dir[currentDir], dir[currentDir ^ 1], CARD_SYSTEM_BLOCK_SIZE);
            updateDir = TRUE;
        } else {
            ASSERTLINE(0x18A, !card->currentFat);

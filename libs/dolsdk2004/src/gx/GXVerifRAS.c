...
void __GXVerifySU(void)
{
    ...
    if (scis_l < 0 && __gxVerif->verifyLevel >= __gxvWarnLev[2]) {
        sprintf(__gxvDummyStr, __gxvWarnings[2], 0);
        __gxVerif->cb(__gxvWarnLev[2], 2U, __gxvDummyStr);
    }

    if (scis_t < 0 && __gxVerif->verifyLevel >= __gxvWarnLev[3]) {
        sprintf(__gxvDummyStr, __gxvWarnings[3], 0);
        __gxVerif->cb(__gxvWarnLev[3], 3U, __gxvDummyStr);
    }

    switch (__gxVerif->rasRegs[67] & 7) {
    case 4:
    case 5:
        if (scis_r > 719 && __gxVerif->verifyLevel >= __gxvWarnLev[4]) {
            sprintf(__gxvDummyStr, __gxvWarnings[4], 719, "YUV");
            __gxVerif->cb(__gxvWarnLev[4], 4U, __gxvDummyStr);
        }

        if (scis_b > 575 && __gxVerif->verifyLevel >= __gxvWarnLev[5]) {
            sprintf(__gxvDummyStr, __gxvWarnings[5], 575, "YUV");
            __gxVerif->cb(__gxvWarnLev[5], 5U, __gxvDummyStr);
        }
        break;
    case 0:
    case 1:
    case 3:
        if (scis_r > 639 && __gxVerif->verifyLevel >= __gxvWarnLev[4]) {
            sprintf(__gxvDummyStr, __gxvWarnings[4], 639, "RGB");
            __gxVerif->cb(__gxvWarnLev[4], 4U, __gxvDummyStr);
        }

        if (scis_b > 527 && __gxVerif->verifyLevel >= __gxvWarnLev[5]) {
            sprintf(__gxvDummyStr, __gxvWarnings[5], 527, "RGB");
            __gxVerif->cb(__gxvWarnLev[5], 5U, __gxvDummyStr);
        }
        break;
    case 2:
        if (scis_r > 639 && __gxVerif->verifyLevel >= __gxvWarnLev[4]) {
            sprintf(__gxvDummyStr, __gxvWarnings[4], 639, __data_0);
            __gxVerif->cb(__gxvWarnLev[4], 4U, __gxvDummyStr);
        }

        if (scis_b > 263 && __gxVerif->verifyLevel >= __gxvWarnLev[5]) {
            sprintf(__gxvDummyStr, __gxvWarnings[5], 263, __data_0);
            __gxVerif->cb(__gxvWarnLev[5], 5U, __gxvDummyStr);
        }
        break;
    }
...
void __GXVerifyPE(void)
{
    ...
    if (__gxVerif->verifyLevel >= 1 && GET_REG_FIELD(__gxVerif->rasRegs[0x41], 1, 0) && GET_REG_FIELD(__gxVerif->rasRegs[0x41], 1, 1) && __gxVerif->verifyLevel >= __gxvWarnLev[0x2F]) {

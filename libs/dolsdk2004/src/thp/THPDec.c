...
const char* __THPVersion = "<< Dolphin SDK - THP\trelease build: Aug 27 2002 20:42:01 >>";
...
void __THPDecompressiMCURowNxN(void)
{
	...
	LCStoreData(__THPInfo->dLC[0], __THPLCWork640[0],
	            ((4 * sizeof(u8) * 64) * (x / 16)));
	LCStoreData(__THPInfo->dLC[1], __THPLCWork640[1],
	            ((sizeof(u8) * 64) * (x / 16)));
	LCStoreData(__THPInfo->dLC[2], __THPLCWork640[2],
	            ((sizeof(u8) * 64) * (x / 16)));

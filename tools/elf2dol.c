...
void read_elf_segments(DOL_map *map, const char *elf, uint32_t sdata_pdhr, uint32_t sbss_pdhr, const char *uses_sbss2)
{
	int read, i;
	Elf32_Ehdr ehdr;
	int usesSbss2 = !(strcmp(uses_sbss2, "yes")) ? 1 : 0;
			...
			if(memsz) {
				...
				if(flags & PF_X) {
					...
					if(filesz == 0) {
						...
						if(i == sbss_pdhr) {
							sbssSizes[0] = memsz;
						} else if(usesSbss2 && i == sbss_pdhr + 2) {
							sbssSizes[1] = memsz;
						}
					} else {
						// DATA segment
						...
						if(i == sdata_pdhr) {
							sdataSizes[0] = memsz;
						} else if(usesSbss2 && i == sdata_pdhr + 2) {
							sdataSizes[1] = memsz;
						}
...
int main(int argc, char **argv)
{
	...
	const char *uses_sbss2 = arg[4];

	DOL_map map;

	memset(&map, 0, sizeof(map));

	read_elf_segments(&map, elf_file, sdata_pdhr, sbss_pdhr, uses_sbss2);
	map_dol(&map);

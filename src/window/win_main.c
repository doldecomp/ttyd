...
void winMain(void) {
    ...
    if (!(wp->flags & 1) && seqGetSeq() == SEQ_GAME && !seqCheckSeq()) {
        ...
        switch (wp->menuState) {
			...
			case 201:
                if (!evtCheckID(wp->currentEvent->eventId)) {
                    wp->menuState = 3;
                }
                break;

            case 300:
                wp->currentEvent = evtEntryType(&evt_diary_read, 0, 0, 0);
                wp->menuState += 1;
                break;
        ...
        if (wp->menuState == 2) {
            s8 characterId = marioGetPtr()->characterId;
            if (characterId < 1) {
                if (characterId < 0) {
                    return;
                }
